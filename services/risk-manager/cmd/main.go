...
func main() {
	...
	// #2: Create and wire CorrelationTracker
	correlationTracker := risk.NewCorrelationTracker(
		correlationEngine,
		cfg.MaxCorrelatedPositions,
		natsPublisher,
		log,
	...
	// #2: Create and wire BatasiWinMonitor
	batasiMonitor := risk.NewBatasiWinMonitor(
		cfg.BatasiWinThreshold,
		cfg.BatasiWinCooldownMin,
		postgresRepo,
		natsPublisher,
		log,
	)
	batasiMonitor.SetStateBuilder(stateBuilder)

	// #2: Create and wire MetabolicMonitor
	metabolicMonitor := risk.NewMetabolicMonitor(
		cfg.MetabolicCPUPercent,
		cfg.MetabolicMemoryBytes,
		cfg.MetabolicGoroutines,
		natsPublisher,
		log,
	)

	// Wire monitors into PitBoss
	pb.SetCorrelationTracker(correlationTracker)
	pb.SetBatasiMonitor(batasiMonitor)
	pb.SetMetabolicMonitor(metabolicMonitor)

	// #26: Start correlation engine background ticker
	correlationEngine.Start(ctx)
	defer correlationEngine.Stop()

	// #2: Start metabolic monitor
	metabolicMonitor.Start(ctx)
	defer metabolicMonitor.Stop()
	...
	if err := natsSubscriber.SubscribeRiskAlert(ctx, func(event ports.RiskAlert) {
		...
		// #15: Only reset counter on successful reconciliation (recon_success event).
		// Non-critical alerts must NOT reset the corruption counter.
	}); err != nil {
	...
	// #7: Subscribe to risk commands from API gateway for state sync
	if err := natsSubscriber.SubscribeRiskCommand(ctx, func(cmd map[string]interface{}) {
		command, _ := cmd["command"].(string)
		switch command {
		case "emergency_stop":
			reason, _ := cmd["reason"].(string)
			if err := emergencyStop.Activate(ctx, reason); err != nil {
		...
		case "parameters_updated":
			...
			log.Info("received parameters_updated command from API gateway")
		case "recon_success":
			// #15: Reset corruption counter only on successful reconciliation
			reconMismatchCount.Store(0)
			log.Info("reconciliation successful, mismatch counter reset")
		default:
			log.Warn("unknown risk command", zap.String("command", command))
		}
	}); err != nil {
		log.Fatal("failed to subscribe to risk command", zap.Error(err))
	}

	wg.Add(1)
	go func() {
		defer wg.Done()
		runStateRefreshLoop(ctx, pb, stateBuilder, redisWriter, natsPublisher, emergencyStop, correlationTracker, batasiMonitor, cfg, log)
	}()

	apiServer := startAPIServer(cfg, pb, emergencyStop, redisWriter, postgresRepo, natsPublisher, correlationEngine, batasiMonitor, log)

	// #1: Separate metrics server on different port
	// #19: MetricsBindAddress defaults to 0.0.0.0 (all interfaces).
	// For production, set RISK_METRICS_BIND=127.0.0.1 to restrict to localhost.
...
func runStateRefreshLoop(
	...
	ct *risk.CorrelationTracker,
	bw *risk.BatasiWinMonitor,
	cfg *config.Config,
	log *zap.Logger,
) {
	...
	for {
		...
		case <-ticker.C:
			...
			// #1: Populate win streak from BatasiWinMonitor before building state
			if bw != nil {
				bwState := bw.GetState()
			...
			// #19: Update CorrelatedPositionsTotal metric
			if ct != nil {
				corrStates := ct.GetCorrelationState()
				for groupID, cs := range corrStates {
					metrics.CorrelatedPositionsTotal.WithLabelValues(metrics.SafeLabel(groupID)).Set(float64(cs.OpenPositions))
				}
			}
...
func startAPIServer(cfg *config.Config, pb *pitboss.PitBoss, es *emergency.EmergencyStop, rw *adapters.RedisWriter, repo *adapters.PostgresRepo, pub *adapters.NATSPublisher, ce *risk.CorrelationEngine, bw *risk.BatasiWinMonitor, log *zap.Logger) *http.Server {
	...
	// #18: API endpoint for correlation groups
	mux.HandleFunc("/api/v1/risk/correlation/groups", jwtAuthMiddleware(cfg.JWTSecret, func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			http.Error(w, `{"error":"method not allowed"}`, http.StatusMethodNotAllowed)
			return
		}
	...
	// #18: API endpoint for batasi win state
	mux.HandleFunc("/api/v1/risk/batasi/state", jwtAuthMiddleware(cfg.JWTSecret, func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			http.Error(w, `{"error":"method not allowed"}`, http.StatusMethodNotAllowed)
			return
		}
	...
	// #18: API endpoint for batasi win manual resume
	mux.HandleFunc("/api/v1/risk/batasi/resume", jwtAuthMiddleware(cfg.JWTSecret, func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			http.Error(w, `{"error":"method not allowed"}`, http.StatusMethodNotAllowed)
			return
		}
		...
		if !resumed {
			http.Error(w, `{"error":"batasi win is not paused"}`, http.StatusBadRequest)
			return
		}
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]string{"status": "resumed"})
