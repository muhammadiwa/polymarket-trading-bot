...
func (w *RedisWriter) WriteState(state ports.PitBossState, ttl time.Duration, emergencyTTL time.Duration) error {
	...
	// #9: Write individual keys for new state fields
	corrExceededVal := "false"
	for _, exceeded := range state.CorrelationExceeded {
		if exceeded {
			corrExceededVal = "true"
			break
		}
	}
	pipe.Set(ctx, "pqap:risk:correlation_exceeded", corrExceededVal, ttl)

	for groupID, exceeded := range state.CorrelationExceeded {
		key := fmt.Sprintf("pqap:risk:correlation:%s", sanitizeRedisKeyComponent(groupID))
		if exceeded {
			pipe.Set(ctx, key, "true", ttl)
		} else {
			pipe.Set(ctx, key, "false", ttl)
		}
	}
	...
	pipe.Set(ctx, "pqap:risk:batasi_win_paused", batasiVal, ttl)

	pipe.Set(ctx, "pqap:risk:win_streak_current", fmt.Sprintf("%d", state.WinStreakCurrent), ttl)
	pipe.Set(ctx, "pqap:risk:win_streak_threshold", fmt.Sprintf("%d", state.WinStreakThreshold), ttl)

	metabolicVal := "false"
	if state.MetabolicAlert {
		metabolicVal = "true"
	}
	pipe.Set(ctx, "pqap:risk:metabolic_alert", metabolicVal, ttl)
