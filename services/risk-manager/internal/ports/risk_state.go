...
type PitBossState struct {
	...
	CorrelationExceeded    map[string]bool            `json:"correlation_exceeded"`
	BatasiWinPaused        bool                       `json:"batasi_win_paused"`
	WinStreakCurrent       int                        `json:"win_streak_current"`
	WinStreakThreshold     int                        `json:"win_streak_threshold"`
	MetabolicAlert         bool                       `json:"metabolic_alert"`
...
type CorrelationGroupRepository interface {
	UpsertCorrelationGroup(ctx context.Context, group CorrelationGroupData) error
	GetCorrelationGroups(ctx context.Context) ([]CorrelationGroupData, error)
	DeleteCorrelationGroup(ctx context.Context, id string) error
}

type CorrelationGroupData struct {
	ID              string    `json:"id"`
