...
def _validate_uuid(account_id: str) -> str:
...
async def create_account(
    ...
    async with pool.acquire() as conn:
        ...
        try:
            ...
            # Rollback: delete the account if encryption fails
            await account_repo.delete_account(conn, account_id)
...
async def get_account(
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        account = await account_repo.get_account_by_id(conn, account_id)
...
async def update_account(
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        account = await account_repo.update_account(conn, account_id, name=request.name)
...
async def deactivate_account(
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        account = await account_repo.set_account_active(conn, account_id, is_active=False)
...
async def activate_account(
    ...
    pool = await get_pool()
    async with pool.acquire() as conn:
        account = await account_repo.set_account_active(conn, account_id, is_active=True)
