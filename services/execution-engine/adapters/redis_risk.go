...
func NewRedisRisk(url string, logger *zap.Logger) (*RedisRisk, error) {
...
func (r *RedisRisk) CheckRisk(ctx context.Context, marketID, strategyID string, orderSize decimal.Decimal) (*ports.RiskDecision, error) {
	...
	if err != redis.Nil {
		...
		if parseErr == nil && marketPos.GreaterThan(decimal.NewFromInt(10000)) {
			return &ports.RiskDecision{
				Allowed: false,
	...
	if err != redis.Nil {
		...
		if parseErr == nil && strategyPos.GreaterThan(decimal.NewFromInt(5000)) {
			return &ports.RiskDecision{
				Allowed: false,
